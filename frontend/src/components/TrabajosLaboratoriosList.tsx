...
const TrabajosLaboratoriosList: React.FC = () => {
    ...
    const [selectedFormWorkId, setSelectedFormWorkId] = useState<number | null>(null);
    ...
    const [searchTerm, setSearchTerm] = useState(() => sessionStorage.getItem('trabajos_search_term') || '');
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 10;

    const handleSearchChange = (term: string) => {
        setSearchTerm(term);
        sessionStorage.setItem('trabajos_search_term', term);
        setCurrentPage(1);
    };
    ...
    const fetchTrabajos = async () => {
        try {
            const response = await api.get('/trabajos-laboratorios?limit=10000');
        ...
        <div className="content-card">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 no-print gap-4">
                <div className="flex items-center gap-3">
                    <div className="p-3 bg-[#3498db]/10 dark:bg-blue-900/50 text-[#3498db] dark:text-blue-400 rounded-2xl shadow-sm">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5 font-medium">
                            Seguimiento de prótesis, envíos y fechas de entrega con laboratorios
                        </p>
                    </div>
                </div>
            ...
            <div className="mb-6 flex flex-wrap gap-4 items-center justify-between bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 no-print">
                <div className="flex items-center gap-2 flex-grow max-w-md">
                    <div className="relative flex-grow">
                        <input
                            type="text"
                            placeholder="Ej: Buscar por paciente o laboratorio..."
                            value={searchTerm}
                    ...
                    {searchTerm && (
                        <button
                            ...
                            className="px-3 py-2 bg-gray-500 hover:bg-gray-600 text-white font-bold rounded-lg shadow-sm transition-all text-xs flex items-center gap-1 shrink-0 transform hover:-translate-y-0.5 active:scale-95"
                ...
                <div className="flex gap-2 ml-auto">
                    ...
                    <button
                        ...
                        className="bg-purple-500 hover:bg-purple-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md transition-all transform hover:-translate-y-0.5 flex items-center gap-2"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>
                        Ubicación de Cubetas
                    </button>
                </div>
            </div>
                            ...
                            <tr key={trabajo.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td className="p-3 flex gap-2">
                                    ...
                                    <button
                                        onClick={() => {
                                            if (trabajo.pagado !== 'si') {
                                                setSelectedFormWorkId(trabajo.id);
                                                setIsFormOpen(true);
                                            }
                                        }}
                                        disabled={trabajo.pagado === 'si'}
            ...
            />
            {/* Manual Modal */}
            {/* TrabajosLaboratoriosForm Drawer */}
            <TrabajosLaboratoriosForm
                isOpen={isFormOpen}
                onClose={() => setIsFormOpen(false)}
                id={selectedFormWorkId}
                onSaveSuccess={() => {
                    fetchTrabajos();
                    setIsFormOpen(false);
                }}
