...
const PersonalList: React.FC = () => {
    ...
    const fetchPersonal = async () => {
        try {
            ...
            setPersonal(Array.isArray(response.data.data) ? response.data.data : []);
    ...
    const handleDelete = async (id: number) => {
        const result = await Swal.fire({
            ...
            text: 'El personal pasará a estado Inactivo.',
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire({
                    title: '¡Dado de baja!',
                    text: 'El personal ha sido desactivado.',
                    icon: 'success',
                ...
                Swal.fire('Error', 'No se pudo desactivar el personal', 'error');
            }
        }
    };
    ...
    const handleReactivate = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire({
                    title: '¡Reactivado!',
                    text: 'El personal ha sido reactivado.',
                    icon: 'success',
                    showConfirmButton: false,
                    timer: 1500
                });
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5 font-medium">
                            Gestión de asistentes, recepcionistas, administradores y personal clínico
                        </p>
                    </div>
                </div>
            ...
            <div className="mb-6 flex flex-wrap gap-4 items-center justify-between bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 no-print">
                <div className="flex items-center gap-2 flex-grow max-w-md">
                    <div className="relative flex-grow">
                        <input
                            type="text"
                            placeholder="Buscar por nombre, paterno, materno o CI..."
                            value={searchTerm}
                            onChange={(e) => {
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Cargo</th>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {personal.map((p, index) => (
                            <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td className="p-3 text-gray-700 dark:text-gray-300 font-medium">{`${p.paterno || ''} ${p.materno || ''} ${p.nombre || ''}`}</td>
                                <td className="p-3 text-gray-700 dark:text-gray-300">{p.ci}</td>
                                <td className="p-3 text-gray-700 dark:text-gray-300">{p.celular}</td>
                                <td className="p-3 text-gray-700 dark:text-gray-300">{p.personalTipo?.area || 'N/A'}</td>
                                <td className="p-3">
                                    <span className={`px-2 py-1 rounded text-sm font-medium ${p.estado === 'activo'
