...
const RecordatorioPlanModal: React.FC<RecordatorioPlanModalProps> = ({ isOpen, onClose, proformaId, proforma }) => {
    ...
    useEffect(() => {
        if ((proforma?.fecha || proformaId) && dias >= 0) {
            const baseDateStr = proforma?.fecha || new Date().toISOString().split('T')[0];
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full transform transition-all animate-fade-in-down border border-gray-200 dark:border-gray-700">
                ...
                <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 rounded-b-xl flex justify-end gap-3">
                    <button
                        onClick={onClose}
                        className="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg font-medium shadow-md transition-all transform hover:-translate-y-0.5"
                        disabled={loading}
                    >
                        Cancelar
                    </button>
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                ...
                                {existingRecordatorio ? 'Actualizar' : 'Guardar Recordatorio'}
                            </>
                        )}
                    </button>
                </div>
