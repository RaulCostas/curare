...
import CategoriaPacienteForm from './CategoriaPacienteForm';
...
const CategoriaPacienteList: React.FC = () => {
    ...
    // Modal state for CategoriaPacienteForm
    ...
    const exportToExcel = () => {
        try {
            const excelData = categorias.map(c => ({
                'ID': c.id,
                'Sigla': c.sigla,
            ...
            XLSX.utils.book_append_sheet(wb, ws, 'CategoriasPaciente');
            XLSX.writeFile(wb, `CategoriasPaciente_${new Date().toISOString().split('T')[0]}.xlsx`);
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const exportToPDF = () => {
        try {
            const doc = new jsPDF();
            doc.text('Lista de Categorías de Pacientes', 14, 15);
            ...
            autoTable(doc, {
                startY: 20,
                head: [['ID', 'Sigla', 'Descripción', 'Color', 'Estado']],
                body: tableData,
            });
            doc.save(`CategoriasPaciente_${new Date().toISOString().split('T')[0]}.pdf`);
        } catch (err) {
            console.error(err);
        }
    };
    ...
    return (
        <div className="content-card p-6 bg-gray-50 dark:bg-gray-800 min-h-screen">
                ...
                <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                            Administrar categorías y listas de precios preferenciales para pacientes
                        </p>
                    </div>
                </div>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {Array.isArray(categorias) && categorias.map((categoria, index) => (
                            <tr key={categoria.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td className="p-3 text-gray-700 dark:text-gray-300 font-medium">{categoria.sigla}</td>
                                <td className="p-3 text-gray-700 dark:text-gray-300">{categoria.descripcion}</td>
                                <td className="p-3">
                                ...
                                <td className="p-3">
                                    <span className={`px-2 py-1 rounded text-sm font-medium ${categoria.estado === 'activo'
            ...
            {/* Manual Modal */}
            {/* CategoriaPacienteForm Modal */}
            <CategoriaPacienteForm
                isOpen={isFormOpen}
