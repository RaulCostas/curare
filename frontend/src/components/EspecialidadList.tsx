...
const EspecialidadList: React.FC = () => {
    ...
    // Modal state for EspecialidadForm
    ...
    const exportToExcel = () => {
        try {
            const excelData = especialidades.map(esp => ({
                'ID': esp.id,
            ...
            XLSX.writeFile(wb, `Especialidades_${new Date().toISOString().split('T')[0]}.xlsx`);
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const exportToPDF = () => {
        try {
            const doc = new jsPDF();
            doc.text('Lista de Especialidades', 14, 15);
            const tableData = especialidades.map(esp => [
                esp.id.toString(),
                esp.especialidad,
                esp.estado || 'activo'
            ]);
            autoTable(doc, {
                startY: 20,
                head: [['ID', 'Especialidad', 'Estado']],
                body: tableData,
            });
            doc.save(`Especialidades_${new Date().toISOString().split('T')[0]}.pdf`);
        } catch (err) {
            console.error(err);
        }
    };
        ...
        <div className="content-card">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 no-print gap-4">
                <div className="flex items-center gap-3 flex-wrap sm:flex-nowrap">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                            Administrar las especialidades clínicas disponibles en el sistema
                        </p>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    ...
                    >
                        <span className="text-xl">+</span> Nueva Especialidad
                    </button>
                </div>
            </div>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {especialidades.map((e, index) => (
                            <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td className="p-3 text-gray-700 dark:text-gray-300 font-medium">{e.especialidad}</td>
                                <td className="p-3">
                                    <span className={`px-2 py-1 rounded text-sm font-medium ${e.estado === 'activo' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'}`}>
                                        {e.estado}
                                    </span>
                                </td>
                                <td className="p-3 flex gap-2">
                                    <button
                                        onClick={() => {
                                            setSelectedEspecialidadId(e.id);
                                            setIsFormOpen(true);
                                        }}
