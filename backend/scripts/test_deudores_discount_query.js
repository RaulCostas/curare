...
async function main() {
    ...
    console.log('=== FAST REALIZED_COST QUERY TEST WITH DISCOUNTS ===\n');
    ...
    const testQuery = `
        ...
        pd_match_by_name AS (
            SELECT DISTINCT ON (pd."proformaId", LOWER(TRIM(COALESCE(a.detalle, ''))))
                   pd."proformaId",
                   LOWER(TRIM(COALESCE(a.detalle, ''))) AS trat_norm,
                   pd.id,
                   pd.total,
                   pd.cantidad
            ...
            ORDER BY pd."proformaId", LOWER(TRIM(COALESCE(a.detalle, ''))), pd.id
        ),
        realized_sum AS (
            SELECT 
                hc."proformaId",
                COALESCE(SUM(
                    CASE 
                        ...
                        -- Match by proformaId + arancel/tratamiento if proformaDetalleId is null
            ...
            LEFT JOIN pd_match_by_name pdm ON (hc."proformaDetalleId" IS NULL AND pdm."proformaId" = hc."proformaId" AND pdm.trat_norm = LOWER(TRIM(COALESCE(hc.tratamiento, ''))))
